...
public class MainDAO_bak {
	...
	public ArrayList<ItemDTO> getJejuList() {
		String sql = "SELECT board.* , (SELECT count(*) FROM tickets_review WHERE ticket_id = board.ticket_id) 이용후기개수				"
				  + " FROM    (SELECT images, best, video_url, available, goods_name, score, price, discount, to_char(price - (NVL((price * discount),0)),'9,999,999') string_discounted_price, ticket_id "
				  + "    	  FROM tickets 																							"
				  ...
				  + " FROM    (SELECT images, best, video_url, available, goods_name, score, price, discount, to_char(price - (NVL((price * discount),0)),'9,999,999') string_discounted_price, restaurant_id "
				  + "    	  FROM restaurants 																						"
				  + "    	  WHERE city = '제주도' AND score IS NOT NULL																"
				  + "    	  ORDER BY best DESC, score DESC) board																	"
