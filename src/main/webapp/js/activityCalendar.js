...
function clickDate(){
    if(event.target.nodeName == "TD" && event.target.textContent !== ""){
        ...
        if(event.target.textContent != date || newDate.getMonth() != month || today){
...
function check_today(index, week, date){
    if(date < today){
        week[index].style.color = 'gray'; //CSS 불러오기
    }
}